...
const Login = () => {
  ...
  const redirectUserBasedOnRole = async (role) => {
    try {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        setToken(res.token), localStorage.setItem("userToken", res.token);
        localStorage.setItem("data", res.data);

        toast.success("Login successful!");
        redirectUserBasedOnRole(res.data.role);
      }
    } catch (error) {
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      setToken(idToken), localStorage.setItem("userToken", idToken);
  ...
  const handleAuthError = (error) => {
    ...
    toast.error(errorMap[error.code] || "An error occurred. Please try again.");
